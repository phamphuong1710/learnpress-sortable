...
class LP_FIB_Upgrade {
	...
	public function __construct() {
		...
		add_action( 'learn-press/admin/page-content-tools/sortable', array( $this, 'admin_tab_content' ) );


		LP_Request::register_ajax( 'sortable-upgrade', array( $this, 'do_upgrade' ) );
	}

	public function admin_tab( $tabs ) {
		$tabs['sortable'] = __( 'Sortable', 'learnpress-fill-in-blank' );

		return $tabs;
	}

	public function admin_tab_content() {
		learn_press_sortable_admin_view( 'tool-upgrade' );
	}
	...
	public function _add_shortcode_id_callback( $a ) {
		if ( ! preg_match_all( '~id=".*"~iSU', $a[0] ) ) {
			return "[sortable " . $a[3] . ' id="' . learn_press_uniqid() . '"]';
		}
	...
	public function get_questions() {
		...
		echo $query = $wpdb->prepare( "
			...
			WHERE post_type = %s
		", '_lp_type', 'sortable', LP_QUESTION_CPT );
